...
### Phase 9: 経営力ドリル統合 (specs/004・005、Wave 1-2 実装済)
- コンセプト: **経営力のゲーミフィケーション**。ゲームは引き出しを「使う」場、ドリルは「研ぐ」場。データのみが行き来する
- ターゲット: 経営力向上を目指す日本のビジネスパーソン全般 (中小企業診断士受験生を含む)
- Wave 1 (生成エンジン、2026-08-01) 実装済: `DrillManager` / `config/drawers.ts` / テスト20件
- Wave 2 (変動費、specs/005、2026-08-06) 実装済: 品質連動の変動費率 / 損益分岐点を `固定費 ÷ 限界利益率` へ是正 / 問題型 `contributionMargin` 追加 / 財務パネルに変動費・限界利益・損益分岐点を表示
  - 難易度は据え置き (売上係数を約1.527倍で補正、クリア率 42% を実測)
  - **8つの引き出しのうち実働3つ → CVP・営業レバレッジが表現可能になった**
- 残: ドリル UI / 問題型の拡充 (NPV・差額原価・税引後CF・営業レバレッジ) / ゲームとの接続 / 変動費率をプレイヤーが下げる手段
...
| 2026-08-01 | **ゲームに変動費が存在しない**ことを技術的最優先課題として認識 | 費用が全額固定のため限界利益率が常に100%になり、CVP・差額原価・営業レバレッジが構造的に表現できない。変動費率の導入はゲームの判断の深さとドリルの正しさに同時に効く（→ 2026-08-06 に解消、specs/005） |
| 2026-08-06 | **変動費率は製品の品質に連動させる**（一律定数にしない） | 一律だと単なる売上係数と同義で意思決定が生まれない。品質連動なら「売上は伸びるが儲けは薄くなる」トレードオフが生じ、製品構成そのものが判断対象になる。PPM・多角化の既存機能にも直結する |
| 2026-08-06 | **変動費導入で難易度は上げない**（売上係数を約1.527倍して補正） | 変動費をそのまま足すと基準品質で利益が約35%消え、既存プレイヤーの体感が一段難化する。構造の入れ替えと難易度調整を同時にやると、どちらが効いたか分からなくなる。クリア率 42% を実測して据え置きを確認 |
| 2026-08-06 | **損益分岐点の正解を `固定費 ÷ 限界利益率` に変更し、旧正解「固定費合計」を罠に降格** | 変動費が無かった頃は固定費合計が正解だったが、それは「このゲームの中でだけ正しい」値。教材を名乗る以上、現実の経営で誤りになる解法を正解として教え続けられない |
