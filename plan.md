...
### Phase 9: 経営力ドリル統合 (specs/004、Wave 1 実装済 2026-08-01)
- コンセプト: **経営力のゲーミフィケーション**。ゲームは引き出しを「使う」場、ドリルは「研ぐ」場。データのみが行き来する
- ターゲット: 経営力向上を目指す日本のビジネスパーソン全般 (中小企業診断士受験生を含む)
- Wave 1 (生成エンジン) 実装済: `DrillManager` / `config/drawers.ts` / テスト20件
- 残: ドリル UI / 問題型の拡充 (NPV・差額原価・税引後CF) / ゲームとの接続
- 価格は買い切りを念頭に置きつつ再検討 (下記 2026-08-01 の決定を参照)
...
| 日付 | 決定 | 理由 |
|------|------|------|
| 2026-08-01 | **経営力ドリルを統合し「経営力のゲーミフィケーション」を製品コンセプトに据える** | 計算科目の学習でつまずいた実体験が起点。原因は知識不足ではなく「どの引き出しを開けるか」の訓練不足。シミュレーションエンジンが罠(誤答)を自動生成できることを垂直スライスで実証 (specs/004) |
| 2026-08-01 | ターゲットは**経営力を高めたいビジネスパーソン**（資格受験生を含むが、受験生向け機能として設計はしない） | ユーザー判断。問題文の主語を「あなたの会社」に統一し、同一エンジンで両者に届かせる |
| 2026-08-01 | 看板は「誤答の自動生成」ではなく**「プレイを経営理論の言葉で振り返らせる」** | 事業性検証の結果。ビジネスゲームが個人向け非同期に降りてこなかった主因はデブリーフの欠落であり、本プロダクトは `theories.ts`「行動→後から理論で命名」と `buildDebrief()` で既に内蔵している。ここが唯一の差別化資産 |
| 2026-08-01 | **ゲームに変動費が存在しない**ことを技術的最優先課題として認識 | 費用が全額固定のため限界利益率が常に100%になり、CVP・差額原価・営業レバレッジが構造的に表現できない。変動費率の導入はゲームの判断の深さとドリルの正しさに同時に効く |
| 2026-08-01 | **課金方針: 買い切りは維持、金額は再検討** (2026-07-30 の「100円」確定を保留に戻す) | ユーザー判断。ドリル統合で提供価値が変わるため、価格は改めて設計する |
| 2026-08-01 | ゲームとドリルを同一画面に混ぜない (ゲーム中にクイズを出さない) | ゲームのテンポが切れ、ドリルは反復の規律を失う。エデュテインメントの典型的失敗を構造的に回避 |
...
| 2026-07-30 | **課金方針: 100円買い切りに確定** | ユーザー判断。広告なし。Android 化 (Phase 7) で Google Play Billing を組み込む前提となる |
| 2026-07-30 | Phase 3 は市場セグメント + PPM 実UI から着手 | ユーザー判断。最後に残った大きなゲーム性の追加を先に完了させ、配信系 (モバイル/シェア) は後回し (specs/003) |
| 2026-05-04 | 課金方針: 100円買い切り or 広告で検討中 | ユーザー判断、Phase 7 で確定 (2026-07-30 に買い切りで確定) |
