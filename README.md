...
## 遊びの骨格 — 緊張感とクリア条件 (v2.5.0)

サンドボックスだった経営に「打ち手を誤るリスク」と「起承転結」を入れたレイヤー ([specs/002](./specs/002-phase2-core-loop.md))。

- **退職・モチベーション**: 過重労働 / 放置 / 研修でモチベーションが動き、下がりきると退職する。隠れ特性 `burnout_prone` / `inconsistent` も効果を持つ
- **オフィス維持費**: オフィスレベル連動の月次固定費。規模を上げるほど損益分岐点が上がるトレードオフ
- **週次ミニイベント**: 決算週以外の各週に1件 (決裁カード / 人事 / 市場)。**全選択肢に「数字への影響」を併記**
- **決裁カードの管理モード注入**: 社長モードの資産を管理モードでも遊べる。理論タグと図鑑解禁もそのまま発火
- **シナリオ「起業1年目」**: 資金を絞った12ヶ月生存チャレンジ。クリア/ゲームオーバー画面と**事後講評** (主要指標 + このプレイで体験した経営理論の一覧)

## 市場セグメントと PPM (v2.5.0)

図鑑の中だけで閉じていた PPM (BCGマトリクス) を、実データで問われる判断に変えるレイヤー ([specs/003](./specs/003-market-segments-ppm.md))。

- **4つの市場セグメント**: エンタープライズ / 中小企業向けSaaS / コンシューマアプリ / AI・データ活用。**成長率は時間で鈍化する**ため「最初に高成長を選ぶだけ」にならない
- **参入市場の選択**: 製品開発時にどの市場で戦うかを選ぶ。選択肢には成長率・自社シェア・競合の厚み・市場規模を明示
- **PPM マトリクス**: 市場タブに 2×2 を実データ描画。縦軸=市場成長率、横軸=相対シェア (最大競合との比、1.00 で並ぶ)。花形 / 金のなる木 / 問題児 / 負け犬に分類し象限ごとの定石を提示
- **撤退**: PPM から直接製品を畳める。売上を失う代わりに開発部の保守負荷が下がり、その市場のシェアは月ごとに失われていく
...
Phase 2 遊びの骨格 + Phase 3 市場セグメント/PPM — PR #41, #43-#48

- **退職・モチベーションの実効化**: 定義済みのまま未使用だった `employeeResignChance` を接続。隠れ特性 `burnout_prone` / `inconsistent` も実効化
- **オフィス維持費**: officeLevel 連動の月次固定費を P/L に計上
- **週次ミニイベント + 決裁カードの管理モード注入**: 決算週以外の各週に1件。理論タグ・図鑑解禁が管理モードでも発火
- **シナリオ「起業1年目」+ 事後講評**: 12ヶ月生存のクリア条件と、体験した理論を振り返る結果画面
- **市場セグメント + PPM 実UI + 撤退**: 多角化と見切りを実データで判断できるようにした
- **テーマのコントラスト是正**: 背景色だけをハードコードした要素が逆テーマで不可視だった問題を修正 (32パターン → 0)。文字用カラートークンを新設
- **検証基盤**: テスト 192 → 331 / 実ブラウザ E2E 10 → 30 チェック。`scripts/contrast-audit.mjs` (コントラスト実測) と `scripts/balance-scenario.mjs` (シナリオのクリア率測定) を追加
