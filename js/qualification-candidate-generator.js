...
/**
 * 求職者に資格を割り当てる
 * @param {Object} candidate - 候補者オブジェクト
 * @returns {string|null} - 資格ID または null
 */
function selectQualificationForCandidate(candidate) {
    // 全体で5%の確率で資格を持つ
 ...
 * 資格保有者の給与を計算
 * @param {Object} candidate - 候補者オブジェクト
 * @param {string|null} qualificationId - 資格ID
 * @returns {number} - 年収（円）
 */
function calculateCandidateSalaryWithQualification(candidate, qualificationId) {
 ...
 * 資格バッジのHTMLを生成
 * @param {string|null} qualificationId - 資格ID
 * @returns {string} - HTMLバッジ
 */
function renderQualificationBadge(qualificationId) {
    if (!qualificationId || !QUALIFICATIONS_30[qualificationId]) {
        return '';
    }
 ...
 * 資格ボーナスの詳細表示を生成
 * @param {string|null} qualificationId - 資格ID
 * @returns {string} - HTML詳細表示
 */
function renderQualificationDetails(qualificationId) {
    if (!qualificationId || !QUALIFICATIONS_30[qualificationId]) {
        return '';
    }
 ...
 */
function getQualificationTierClass(tier) {
    return `tier-${tier}`;
}

/**
 * 資格保有率の統計を取得（デバッグ用）
 * @param {Array} candidates - 候補者配列
 * @returns {Object} - 統計情報
 */
function getQualificationStats(candidates) {
    const stats = {
    ...
    for (const candidate of candidates) {
        if (candidate.qualification) {
            ...
            if (qual) {
                stats.byTier[qual.tier]++;
                stats.byCategory[qual.category] = (stats.byCategory[qual.category] || 0) + 1;
            }
        }
    }

    stats.qualificationRate = (stats.withQualification / stats.total * 100).toFixed(2) + '%';
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        selectQualificationForCandidate,
        calculateCandidateSalaryWithQualification,
        renderQualificationBadge,
        renderQualificationDetails,
        getQualificationTierClass,
